...
## Local vs Production (No Confusion)

Use these rules consistently:

- **Local mode (no domain, HTTP, localhost):**
  - Command: `make up-local`
  - URL: `http://localhost`
  - Do **not** include `docker-compose.https.yml`.

- **VM public IP over HTTP (staging only):**
  - Command: `make up-local`
  - URL: `http://<VM_PUBLIC_IP>`
  - Keep `NODE_ENV=development` and `COOKIE_SECURE=false`.

- **Production mode (HTTPS + domain):**
  - Command: `make up-prod`
  - `docker-compose.prod.yml` is the explicit switch that turns on production-safe runtime behavior.

---

## Production Changes Required (File-by-File)

This section lists exactly what matters in each changed file and what you must do for production.

| File | What changed | What you must do for production |
|---|---|---|
| `docker-compose.yml` | Local-safe defaults (`CORS_ORIGIN=http://localhost`, `COOKIE_SECURE=false`, `APP_URL=http://localhost`, `NODE_ENV=${NODE_ENV:-development}`) | Do not edit this file per deploy. Use `.env` + `docker-compose.prod.yml` to override for production. |
| `docker-compose.prod.yml` | Explicit production overrides | Use only with HTTPS deployment. Keep `NODE_ENV=production`, `COOKIE_SECURE=true`, and `web` on `80:80`. |
| `docker-compose.https.yml` | HTTPS/domain overlay | `DOMAIN` is required; use this file **only** when you have a real domain + certs. |
| `.env` (from `.env.example`) | Holds environment-specific values | Set strong secrets, real `APP_URL`, and production `CORS_ORIGIN`; never commit `.env`. |
| `.env.example` | Local examples and safer placeholders | No runtime action; use it as a template only. |
| `backend/src/server.ts` | Enforces strict CORS in production | Ensure `NODE_ENV=production` and set non-empty `CORS_ORIGIN` in `.env`. |
| `backend/src/auth.ts` | Refuses insecure cookie settings in production | Ensure `COOKIE_SECURE=true` when `NODE_ENV=production`. |
| `backend/scripts/seed.ts` | Rejects weak/default superadmin password | Set a strong `SUPERADMIN_PASSWORD` (minimum 14 chars, non-default). |
| `backend/src/routes/assistant.ts` | API key moved from query string to header | Set `GEMINI_API_KEY` in `.env` if assistant is enabled; rotate key if previously exposed. |
| `backend/Dockerfile` | Uses lockfile + npm 11 + `npm ci --legacy-peer-deps` | No per-deploy edit needed; just rebuild images. |
| `backend/package.json`, `backend/package-lock.json` | Security-driven dependency updates | No manual change required during deploy; keep lockfile committed. |
| `src/lib/api.ts`, `src/lib/hooks.ts`, admin route files | Frontend now relies on cookie/session flow instead of bearer token from localStorage | No deploy-time edits required; behavior is already production-safe. |
...
Set these values for **HTTP-only VM bring-up / staging** (generate strong random strings for secrets):
```bash
...
For **real production** (domain + HTTPS), switch to:
```bash
...
## Part 5 — Production overlay files (already in repo)

This repository already contains `docker-compose.prod.yml` and `docker-compose.https.yml`.
Do not edit application source files for deployment mode changes. Only set `.env` values and use the right command.

Why this helps:

- Local mode uses only `docker-compose.yml`.
- Staging over VM IP uses only `docker-compose.yml`.
- Production mode always includes `docker-compose.prod.yml` and `docker-compose.https.yml`.
- No repeated manual editing of source files when switching environments.

---

## Part 6 — Build and start everything

For HTTP-only staging on VM public IP:
```bash
make up-local
```

For real production (HTTPS + domain):
```bash
make up-prod
```
...
```cron
0 3,15 * * * certbot renew --pre-hook "cd /home/azureuser/app && make down-prod" --post-hook "cd /home/azureuser/app && make up-prod" --quiet
```
